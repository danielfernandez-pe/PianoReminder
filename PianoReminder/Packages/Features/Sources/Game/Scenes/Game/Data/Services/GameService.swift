...
final class GameService {
    ...
    func fetchChord(id: String) async throws -> ChordDAO {
        let data = try await networking.getDocument(path: FirebaseCollection.chords.getPathWithLocale(), id: id)
        let jsonDecoder = JSONDecoder()
        return try jsonDecoder.decode(ChordDAO.self, from: data)
    }

    func fetchNote(id: String) async throws -> SingleNoteDAO {
        let data = try await networking.getDocument(path: FirebaseCollection.notes.getPathWithLocale(), id: id)
        let jsonDecoder = JSONDecoder()
        return try jsonDecoder.decode(SingleNoteDAO.self, from: data)
    }

    func fetchHistory(id: String) async throws -> HistoryDAO {
        let data = try await networking.getDocument(path: FirebaseCollection.history.getPathWithLocale(), id: id)
        let jsonDecoder = JSONDecoder()
        return try jsonDecoder.decode(HistoryDAO.self, from: data)
    }

    func fetchNotes() async throws -> [SingleNoteDAO] {
...
//        ]
    ...
    func fetchChords() async throws -> [ChordDAO] {
...
//        ].flatMap { $0 }.shuffled()
    ...
    func fetchHistoryQuestions() async throws -> [HistoryDAO] {
